    ...
    try {
        ...
        if (photoSelectResult && photoSelectResult.photoUris && photoSelectResult.photoUris.length > 0) {
            ...
            if (imgUri.indexOf('media/Photo') < 0) {
                showToast({ "id": 16777233, "type": 10003, params: [], "bundleName": "com.example.newsrelease", "moduleName": "entry" });
                return '';
            }
        ...
        try {
            ...
            request.uploadFile(context, uploadRequestOptions).then((data: request.UploadTask) => {
                ...
                data.on("fail", (result: Array<request.TaskState>) => {
                    ...
                    if (result && result.length >= 1) {
                        serverData.msg = { "id": 16777250, "type": 10003, params: [], "bundleName": "com.example.newsrelease", "moduleName": "entry" };
                        reject(serverData);
                    }
                });
            }).catch((err: Error) => {
